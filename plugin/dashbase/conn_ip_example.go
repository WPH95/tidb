...
func OnInit(ctx context.Context, manifest *plugin.Manifest) error {
	...
	fmt.Println("read cfg in init", manifest.SysVars["dashbase_example_test_variable"].Value)
	return nil
}
...
func OnTableCreate(ctx context.Context, sctx *variable.SessionVars, event plugin.GeneralEvent, cmd string) {
	fmt.Println("conn_ip_example notifiy called")
	fmt.Println("variable test: ", variable.GetSysVar("dashbase_example_test_variable").Value)
