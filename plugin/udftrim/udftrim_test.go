...
func (ts *TidbTestSuite) SetUpSuite(c *C) {
	manifest := &plugin.UDFManifest{
		...
		GetUserDefinedFuncClass: GetUserDefinedFuncClass,
	}
...
func (s *testPlugin) TestPlugin(c *C) {
	...
	tk.MustExec("create table t(a int, b char(255))")
	tk.MustExec(`insert into t values(1, " lfkdsk "),(2, " lfk dsk ")`)
	tk.MustExec("analyze table t")
	result := tk.MustQuery("select UDF_TRIM(b) from t where a = 1")
	result.Check(testkit.Rows("lfkdsk", ))
}
